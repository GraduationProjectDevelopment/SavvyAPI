...
class BudgetRepository(IBudgetRepository):
    ...
    #     return [
    #         Budget(**item) for item in response.data
    #     ] if response.data else []

    # def get_budget_by_id(self, budget_id: UUID) -> Optional[Budget]:
    #     response = get_supabase().table("budgets").select("*").eq("budget_id", str(budget_id)).single().execute()
    #     return Budget(**response.data) if response.data else None

    # def get_budgets_by_user(self, user_id: UUID) -> List[Budget]:
    #     response = get_supabase().table("budgets").select("*").eq("user_id", str(user_id)).execute()
    ...
    #         "allocated_amount": budget.allocated_amount,
    #     }
    #     response = get_supabase().table("budgets").update(data).eq("budget_id", str(budget.budget_id)).execute()
    #     return budget if response.data else None

    # def delete_budget(self, budget_id: UUID) -> bool:
    #     response = get_supabase().table("budgets").delete().eq("budget_id", str(budget_id)).execute()
